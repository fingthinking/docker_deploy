...
do
	...
	lst=`printf "%.2d" $i`
	docker create --network=${zk_net_name} --ip=${zk_prefix}1${lst} -i -t -v ${share_file}:/mnt -p $[zk_client_port+$i]:${zk_client_port} --name=${zk_server}${i} --hostname=${zk_server}${i} ${image_name}
	echo $i > myid
